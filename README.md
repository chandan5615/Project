...
### **Check Blocked IPs**
```bash
docker exec sentinel-agent sqlite3 /app/data/sentinel_intel.db   "SELECT ip, banned_until, offense_count, status FROM blocked_ips;"
```

### **Check Whitelist**
```bash
docker exec sentinel-agent sqlite3 /app/data/sentinel_intel.db   "SELECT ip, reason, auto_detected FROM safe_ips;"
```
...
docker exec sentinel-agent sqlite3 /app/data/sentinel_intel.db   "SELECT * FROM blocked_ips WHERE ip='ATTACKING_IP';"
...
sleep 120
docker exec sentinel-agent sqlite3 /app/data/sentinel_intel.db   "SELECT ip, status FROM blocked_ips WHERE ip='192.168.1.99';"
...
docker exec sentinel-agent sqlite3 /app/data/sentinel_intel.db   "SELECT ip, banned_until, status FROM blocked_ips WHERE banned_until < datetime('now');"
