...
if [ -f "docker-compose.yml" ]; then
    ...
    # Auto-detect server IP for dashboard binding (security)
    log_info "Auto-detecting server IP address for dashboard binding..."
    
    # Get primary IP using multiple methods (same logic as tools.py)
    ...
    if [ -z "$DETECTED_IP" ] || [ "$DETECTED_IP" = "127.0.0.1" ]; then
        DETECTED_IP=$(ip addr show 2>/dev/null | grep 'inet ' | grep -v '127.0.0.1' | head -1 | awk '{print $2}' | cut -d'/' -f1)
    fi
    
    # Fallback to localhost if no IP detected
    if [ -z "$DETECTED_IP" ]; then
        DETECTED_IP="127.0.0.1"
        log_warning "Could not detect network IP, using localhost (dashboard only accessible from server)"
    else
        log_success "Detected server IP: ${GREEN}$DETECTED_IP${NC}"
        log_info "Dashboard will be accessible at: ${CYAN}http://$DETECTED_IP:8501${NC}"
    fi
    
    # Create/update .env file with auto-detected IP
    log_info "Creating .env file with auto-detected configuration..."
    
    cat > .env << EOF
# Sentinel Agent Environment Configuration
# Auto-generated by AUTO_INSTALL.sh on $(date)

# Dashboard binding - AUTO-DETECTED (restricts to local network only)
...
EOF
    ...
    log_success ".env file created with DASHBOARD_BIND_IP=$DETECTED_IP"
...
# Use the auto-detected IP from earlier
if [ -z "$DETECTED_IP" ]; then
...
${CYAN}Access URLs:${NC}
  📊 Dashboard:  http://$DETECTED_IP:8501 ${GREEN}(Auto-detected IP, local network only)${NC}
  🔌 API:        http://$DETECTED_IP:8000
  💚 Health:     http://$DETECTED_IP:8000/api/health

${CYAN}Security Configuration:${NC}
  🔒 Dashboard port 8501: ${GREEN}Restricted to local network only${NC}
  🔒 Bound to IP: ${GREEN}$DETECTED_IP${NC} (auto-detected)
  🔒 NOT accessible from public internet ✅
  🔐 Authentication: ${YELLOW}Enabled (login required)${NC}
  
  ${YELLOW}💡 If your server IP changes, just re-run:${NC}
     ${CYAN}sudo ./AUTO_INSTALL.sh${NC}
     (Will auto-detect new IP and reconfigure)

${CYAN}Default Credentials:${NC}
  Dashboard Login (auto-generated, check Docker logs for actual password):
    Username: ${GREEN}admin${NC}
    Password: ${YELLOW}See: docker logs sentinel-agent | grep "DEFAULT ADMIN"${NC}
  
  ${RED}⚠️  CHANGE PASSWORD IMMEDIATELY AFTER FIRST LOGIN!${NC}
...
${CYAN}Useful Commands:${NC}
  ...
  Check IP:      ${YELLOW}cat .env | grep DASHBOARD_BIND_IP${NC}
...
${YELLOW}⚠️  IMPORTANT NOTES:${NC}
  ...
  3. Dashboard is ${GREEN}LOCAL NETWORK ONLY${NC} - not accessible from public internet ✅
  4. Change default admin password after first login!
  5. AI analysis only runs for ${GREEN}HIGH severity${NC} attacks (optimized performance)
...
${GREEN}System is monitoring:${NC}
  ...
  ✓ Auto-unblocking (temporary bans with expiry)
  ✓ Whitelist protection (admin IPs protected)
  ✓ Progressive punishment (repeat offenders get longer bans)
