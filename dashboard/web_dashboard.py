...
def get_displayed_credentials() -> str:
    ...
    return "[WARNING] Credentials file not found. Check /app/data/INITIAL_CREDENTIALS.txt or ./data/INITIAL_CREDENTIALS.txt"
...
def render_wall_of_shame(data_manager: DashboardDataManager):
    ...
    if df.empty:
        st.info("[INFO] STATUS: No blocked IPs - Network is clean")
    else:
        # Format for display
        display_df = df.copy()
...
def render_incident_feed(data_manager: DashboardDataManager):
    ...
    if df.empty:
        st.info("[INFO] STATUS: No recent incidents detected - system is running normally")
    else:
        # Format for display
        display_df = df.copy()
...
def render_network_health(data_manager: DashboardDataManager):
    ...
    if stats['minutes_data']:
        ...
        if not chart_df.empty and len(chart_df) > 0:
            ...
            st.warning("[WARNING] No data points available for chart")
    else:
        st.info("[INFO] No incident data available for the last hour - check back later")
...
def render_ip_blocking():
    ...
    with col2:
        ...
        st.info("[WARNING] This will completely remove the IP from firewall AND delete all its incidents from the database.")
    ...
    if st.button(" Refresh Blocked IPs List"):
        try:
            ...
            if error:
                ...
                if blocked:
                    st.success(f"[OK] Loaded {len(blocked)} blocked IP(s) from {firewall_type}")
                else:
                    st.info(f"[INFO] No currently blocked IPs in {firewall_type}")
        except Exception as e:
            st.error(f"[ERROR] Critical error: {e}")
            st.session_state.blocked_ips = []
    ...
    if blocked_ips:
        ...
        st.info(f"[INFO] No blocked IPs found - Select {firewall_type} and click Refresh to load current rules")
...
def render_attack_patterns(data_manager: DashboardDataManager):
    ...
    try:
        ...
        st.error(f"[ERROR] Error analyzing attack patterns: {e}")
        ...
        if st.button("Export Incidents to CSV", type="primary"):
            try:
                ...
                if not df.empty:
                    ...
                    st.session_state.export_csv_name = f"sentinel_incidents_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
                    st.session_state.export_csv_count = len(df)
                else:
                    st.session_state.export_csv = None
                    st.warning("No incidents found for selected time range")
            except Exception as e:
                st.error(f"Export failed: {e}")
                st.session_state.export_csv = None
        
        # Download button OUTSIDE the if st.button block - always rendered when data ready
        if st.session_state.get("export_csv"):
            st.success(f"Ready: {st.session_state.get('export_csv_count', 0)} incidents")
            col_dl, col_cl = st.columns([3, 1])
            with col_dl:
                st.download_button(
                    label="Download CSV",
                    data=st.session_state.export_csv,
                    file_name=st.session_state.get("export_csv_name", "incidents.csv"),
                    mime="text/csv",
            ...
            try:
                ...
                if cursor.fetchone():
                    threat_df = pd.read_sql_query(
                        "SELECT * FROM threat_intel ORDER BY last_checked DESC", conn
                    )
                    if not threat_df.empty:
                        st.session_state.export_json = threat_df.to_json(orient='records', indent=2)
                        st.session_state.export_json_name = f"sentinel_threat_intel_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
                        st.session_state.export_json_count = len(threat_df)
                    else:
                        st.session_state.export_json = None
                        st.warning("No threat intelligence records found")
                else:
                    st.session_state.export_json = None
                    st.warning("Threat intelligence table not found in database")
                conn.close()
            except Exception as e:
                st.error(f"Export failed: {e}")
                st.session_state.export_json = None
        
        # Download button OUTSIDE the if st.button block
        if st.session_state.get("export_json"):
            st.success(f"Ready: {st.session_state.get('export_json_count', 0)} records")
            col_dl, col_cl = st.columns([3, 1])
            with col_dl:
                st.download_button(
                    label="Download JSON",
                    data=st.session_state.export_json,
                    file_name=st.session_state.get("export_json_name", "threat_intel.json"),
                    mime="application/json",
                    key="json_download_btn"
                )
            with col_cl:
                if st.button("Clear", key="clear_json"):
                    st.session_state.export_json = None
                    st.rerun()
...
def _show_login_page():
    ...
    with col2:
        ...
        with st.form("login_form"):
            ...
            with col_btn2:
                help_btn = st.form_submit_button("[Help] Help", use_container_width=True)
            ...
            if submit:
                if not username or not password:
                    ...
                    if not st.session_state.get("authenticator"):
                        st.error("[ERROR] Authentication module not initialized")
                        ...
                        try:
                            ...
                            if success:
                                ...
                                st.success(f"[OK] Welcome, {username}!")
                                st.rerun()
                            else:
                                st.error("[ERROR] Invalid username or password")
                                st.warning("[WARNING] Failed login attempt logged")
                        except Exception as e:
                            st.error(f"[ERROR] Authentication error: {e}")
        ...
        st.markdown("---")
        st.caption(" Sentinel Agent v2.3 | Secure Dashboard Access")
        st.caption("[WARNING] Unauthorized access is monitored and logged")


def render_settings(data_manager):
    """Render settings page for password change and system info"""
    ...
    with st.form("password_change_form"):
        ...
        if st.form_submit_button("Update Password", use_container_width=True):
            ...
            if not all([current_password, new_password, confirm_password]):
                ...
                st.error("[ERROR] New password must be at least 8 characters")
            elif new_password != confirm_password:
                st.error("[ERROR] Passwords do not match")
            elif current_password == new_password:
                st.error("[ERROR] New password must be different from current")
            else:
                try:
                    # Verify current password
                    authenticator = st.session_state.get("authenticator")
                    if authenticator:
                        success, _ = authenticator.authenticate(username, current_password)
                        if success:
                            # Change password
                            success, message = authenticator.change_password(username, current_password, new_password)
                            if success:
                                st.success("[OK] Password changed successfully")
                                st.info("[INFO] Please log in again with your new password")
                                st.session_state.authenticated = False
                                st.rerun()
                            else:
                                st.error(f"[ERROR] {message}")
                        else:
                            st.error("[ERROR] Current password is incorrect")
                    else:
                        st.error("[ERROR] Authentication module not available")
                except Exception as e:
                    st.error(f"[ERROR] Password change failed: {e}")
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.text_input("Database Path", db_path, disabled=True)
    ...
    with col2:
        if st.button("Test Connection", use_container_width=True):
            try:
                ...
                conn.close()
                st.success(f"[OK] Connection OK - {table_count} tables found")
            except Exception as e:
    ...
    try:
        if os.path.exists(db_path):
            ...
            st.warning("[WARNING] Database file not found")
    except Exception as e:
        st.warning(f"[WARNING] Could not read database file: {e}")
    ...
    with col2:
        if st.button("Force Refresh Dashboard", use_container_width=True):
            st.rerun()
    
    # Display active authentication info
    st.info(f"[INFO] Logged in as: {username}")
...
def main():
    ...
    if not st.session_state.auth_initialized:
        try:
            ...
            st.error(f"[WARNING] Authentication module not available: {e}")
    ...
    # Auto-refresh using Streamlit's built-in fragment or meta refresh
